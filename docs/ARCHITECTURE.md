...
- **No backend server**: There is no Express (or other) API server in normal use. Auth and data are handled by **Firebase** (Auth + Firestore + Storage).
...
1. **Public**
   ...
   - Request form: `/request` — submits directly to **Firestore** (and optional file uploads to **Firebase Storage**). No server in the middle.
...
3. **Security**
   ...
   - **Storage rules**: Anyone can **create** files under `requests/{requestId}/` (with size limit). Only signed-in users can read/delete.
